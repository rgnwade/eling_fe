...
@section('content')
    <section class="checkout" id="app-checkout">
                        ...
                        <div class="cart-total">
                            ...
                            <span class="item-amount">
                                {{ trans('storefront::cart.weight_total') }}
                                  <span >{{ $cart->getWeightTotal() }}  Kg</span>
                            </span>
                            <span class="item-amount">
                                {{ trans('storefront::cart.shipping_total') }}
                                  <span id="shipping-total-amount">{{ $cart->shippingCost()->convertToCurrentCurrency()->format() }}</span>
                                  <input type="hidden" value="{{$cart->shippingCost()}}" id="shipping_total_value">
                            </span>


                            {{-- @if ($cart->hasAvailableShippingMethod())
                                <div class="available-shipping-methods"> 2
                            ...
                            <div class="checkout-terms checkbox text-center">
                                ...
                                <label for="terms-and-conditions">
                                    {{ trans('storefront::checkout.i_agree_to_the') }} <a href="{{ $termsPageURL }}"  target="_blank" >{{ trans('storefront::checkout.terms_and_conditions') }}</a>
                                </label>
                            </div>
    ...
    <script src="{{ v(Theme::url('public/js/app-vue.js')) }}"></script>
@endsection
