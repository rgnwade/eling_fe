...
<body class="file-manager">
    <div class="container">
        @if($type == 'image')
        @include('media::vendor.media.partials.uploader')
        @else()
        @include('media::vendor.media.partials.uploaderFile')
        @endif

        <div class="row">
            <div class="col-md-12">
                <div class="box box-primary">
                    @include('media::vendor.media.partials.table')
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        DataTable.setRoutes('.file-manager .table', {
            index: {
                name: 'vendor.media.index',
                params: { type: '{{ $type }}' }
            },
            destroy: 'vendor.media.destroy',
        });
